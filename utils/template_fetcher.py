...
def get_template_details(config: dict, template_id: int, log_callback=print) -> dict:
    """
    Fetches the full details for a single template, including custom fields.
    """
    if not template_id:
        return {}
    ...
    if response and response.status_code == 200:
        ...
        log_callback(f"✅ Found details for Template ID: {template_id}")
        return template_details
    else:
        log_callback(f"❌ Error fetching details for Template ID {template_id}.")
        return {}